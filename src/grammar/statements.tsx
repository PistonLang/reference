...
const _defs: Record<keyof typeof stmts, GrammarPoints> = {
    FunctionParam: [types.Identifier, tokens.colon, types.Type],
    FunctionParams: [tokens.lParen, many(stmts.FunctionParam, tokens.commaOrNL), option(stmts.FunctionParam), tokens.rParen],
    PropertyDef: [union(tokens.valKw, tokens.varKw), types.Identifier, tokens.colon, types.Type, option(tokens.eq, exprs.Expression)],
    Statement: union(stmts.ClassDef, stmts.TraitDef, stmts.PropertyDef, stmts.GetterDef, stmts.SetterDef),
    FunctionLikeDef: [types.Identifier, option(types.TypeParams), stmts.FunctionParams, tokens.arrow, types.Type, types.WhereClause, option(tokens.eq, exprs.Expression)],
    ...
    SuperTypes: [tokens.subtype, types.Type, many(tokens.comma, types.Type)],
